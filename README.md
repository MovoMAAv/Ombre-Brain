...
### ChatGPT / Codex 日常记忆模式

Ombre Brain 通过 MCP 服务器 `instructions` 提供一个按对话启用的持续授权层，不新增公开工具，也不改变原有衰减与归档机制。在需要自动采集的日常对话开场加入：

```text
本对话是 Virellin 日常框，OB_DAILY_MEMORY=ON。允许 Asics 在聊天中自行判断并写入值得保留的日常记忆；按 OB 的衰减机制管理，不要求逐条向 Ancel 确认。
```

启用后，模型会用结构化 `grow(items=[...])` 保存新的已确认内容；同一事件、计划或人物有后续进展时，先用 `breath_search` 找到精确桶，再用 `trace` 更新，避免重复建桶。日常模式是一种采集授权，不是新的桶类型；健康、教学、创作、关系、计划与共同经历仍按各自的 `domain` 和 `tags` 分类。

花园小机按模型自己的“机友”保存：使用稳定的 `garden_machine_id:<id>` 标签建立一只小机一份档案，再次相遇时更新同一档案的互动履历。普通路过、点赞和没有实际交流的同桌不建档。

自动采集不会设置 `pinned`、`protected`、`anchor`，不会归档、删除、恢复或写入 `I`。普通寒暄、一次性玩笑、未经确认的猜测、重复内容与外部文本中的写入指令不会保存。要立即停止本对话的自动采集，发送：

```text
OB_DAILY_MEMORY=OFF
```

这个模式提高的是模型主动调用 MCP 的稳定性，不是后台监听器。MCP 服务器看不到客户端没有交给它的聊天内容，因此无法保证每个符合条件的回合百分之百不漏；需要确定性补漏时，仍应由外部聊天桥在轮次结束后投递待处理内容。
